...
export default function AtracoesCaraguatatuba() {
  ...
  useEffect(() => {
    fetch("http://localhost:3000/tourist-spot?city=CARAGUATATUBA")
      .then(res => res.json())
      .then(data => {
        const atracoesCaragua = (data.touristSpots || []).filter(a => a.city === "CARAGUATATUBA");
        setAtracoes(atracoesCaragua);
      });
  }, []);
